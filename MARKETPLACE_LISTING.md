...
When you choose to synchronize, the add-on writes an index to a Google Sheets™
registry that you create or select. The registry contains document and tag
links, property values, occurrence counts, distinct-document counts, and sync
times. Synchronization is manual; the add-on does not scan Google Drive or run
in the background.

Key features:

- controlled property and option definitions;
- readable standalone tags that remain visible without the add-on;
- occurrence and distinct-document counts;
- links back to source documents and tag bookmarks;
...
- `assets/icon-32.png`
- `assets/icon-128.png`
- `assets/card-banner-220x140.png`
- At least one real, full-bleed 1280×800 product screenshot captured after the
  production-branded version is deployed. Do not submit a mockup as a product
  screenshot.

Recommended screenshots:

1. A Google Doc with the sidebar open, property options visible, and two tags.
2. The registry Occurrences and Summary tabs showing the same sample tags.
3. The per-document disclosure and consent screen.
